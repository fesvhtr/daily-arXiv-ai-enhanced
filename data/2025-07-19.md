...
Conclusion: EaGERS为DocVQA任务提供了一种高效、透明的解决方案，无需额外训练。
...
Method: MindJourney通过迭代生成相机轨迹并合成多视角图像，结合VLM进行推理。

Result: 在SAT基准测试中平均提升8%性能，优于强化学习训练的VLM。
...
Method: 通过嵌入视觉参数空间和delta调优，设计Mono-InternVL模型，并采用创新的EViP预训练策略。改进版Mono-InternVL-1.5引入视觉注意力专家和高效预训练重组。

Result: Mono-InternVL在15个基准测试中优于现有模型，如OCRBench上提升114分。Mono-InternVL-1.5在保持性能的同时显著降低了成本。

Conclusion: 提出的方法有效解决了单模态MLLM的优化问题，并通过改进版进一步提升了效率和性能。
...
Result: 研究发现，精细预处理与Transformer模型结合表现最佳；迁移学习提高了工作流程适应性；工作流程选择取决于标记样本的可用性。

Conclusion: 监督训练在样本充足时更准确，而迁移学习在样本不足时是可行的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究放射科医生在CT阅读中的眼动行为对开发可解释的计算机辅助诊断系统至关重要，但现有研究因缺乏公开数据集和CT体积的3D复杂性而受限。

Method: 提出CT-ScanGaze数据集，并设计CT-Searcher预测器处理3D CT体积生成放射科医生类似的3D注视序列，同时开发了将2D眼动数据转换为3D的预训练流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法未能捕捉树结构的语义层次关系，因此提出了一种更符合自然树结构的分层多尺度融合方法。

Method: 采用分层多尺度特征提取（局部、分支和冠层尺度）和跨尺度信息传播，结合特定尺度的特征工程。

Result: 在树种类分类和3D物体识别任务中，MS-DGCNN++均优于现有方法，准确率最高达94.96%。

Conclusion: MS-DGCNN++不仅高效且适用于资源受限场景，还能泛化到多种点云处理任务。
...
<details>
  <summary>Details</summary>
Motivation: 解决人-物交互检测中数据长尾分布问题，强调在编码器阶段预判HOI特定线索以获得更强场景理解。

Method: 提出Funnel-HOI框架，先探测物体再关联动作，利用非对称共注意力机制挖掘多模态信息，设计新型损失函数优化分类。

Result: 在HICO-DET和V-COCO数据集上，未见和罕见HOI类别分别提升12.4%和8.4%。

Conclusion: Funnel-HOI通过编码器阶段的预判和多模态信息挖掘，显著提升了HOI检测性能。
...
Result: 发现视网膜动脉宽度、血管密度与特定脂质亚类（如TAGs、DAGs和Cers）之间存在强相关性，提示代谢压力下的微血管重塑机制。

Conclusion: 该研究为早期CVD发病机制提供了新见解，并为非侵入性生物标志物的开发及个性化心血管医疗提供了潜在机会。
...
Conclusion: NeuraLeaf为植物建模提供了有效工具，其实现和数据集已公开。
...
<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在空间定位领域取得了显著进展，但缺乏对当前方法、数据集、评估指标及工业应用的全面总结。

Method: 通过系统性文献综述，分析了Transformer模型在空间定位中的应用，总结了主流架构、数据集和评估指标。

Result: 研究揭示了主要方法趋势和最佳实践，为开发可靠且适用于工业的模型提供了指导。
...
Method: 采用双分支架构整合视觉（车载视频）和文本（事故报告）数据，结合多模态特征聚合和大模型（如GPT-4o、Long-CLIP），并利用提示工程生成标准化反馈。

Result: 在多个基准数据集（DAD、CCD、A3D）上验证了方法的优越性，包括更高的预测准确性、响应速度、计算效率和可解释性。

Conclusion: 该框架为交通事故预测领域设定了新的性能标准，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 视频发型转移在社交媒体、游戏等领域需求增长，但现有方法在时间一致性和动态适应性方面存在挑战。
...
Method: 提出Mamba Snake框架，结合状态空间建模和蛇形演化，引入Mamba Evolution Block（MEB）和能量图形状先验，优化多尺度轮廓演化。
...
<details>
  <summary>Details</summary>
Motivation: 动态海洋环境中实时跟踪海洋动物（如鲨鱼）具有挑战性，传统方法受限于单无人机电池寿命和环境干扰。

Method: 系统结合机载计算机、RGB-D相机和自定义OSTrack管道，支持多无人机无缝交接跟踪任务。

Result: 在5200帧鲨鱼数据集上测试，实时跟踪成功率达81.9%，多无人机交接覆盖率达82.9%。

Conclusion: 该系统证明了多无人机协作在海洋监测中的可行性，为未来自主监测系统奠定了基础。
...
Method: 引入ATARA框架加速数据收集，并设计AnyPos逆向动力学模型解决数据分布不匹配问题。
...
Result: 实验显示ReToM在FID和CLIP分数上分别提升了6.2%，同时保持推理时间相近。
...
<details>
  <summary>Details</summary>
Motivation: 现代深度学习中的Transformer架构在计算机视觉中广泛应用，但计算成本高，需更高效的实现。

Method: 提出KCR-Transformer，在MLP层进行通道选择，基于理论泛化边界优化计算效率。

Result: 实验表明，KCR-Transformer在减少FLOPs和参数的同时，性能优于原模型。

Conclusion: KCR-Transformer为高效视觉Transformer提供了理论和实践支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型（LVLMs）主要针对室内场景，难以适应户外大尺度场景的多视角、多模态需求，且缺乏多域感知数据。

Method: 构建SVM-City数据集（包含多尺度、多视角和多模态数据），并设计City-VLM模型，通过联合概率分布空间实现多模态融合。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档质量评估方法准确性不足，限制了实际应用。MLLM在图像质量评估中的成功启发了将其扩展到文档领域的尝试。

Method: 提出DeQA-Doc框架，利用MLLM的视觉语言能力和软标签策略，调整分辨率限制并引入集成方法。

Result: 实验表明DeQA-Doc在多种退化类型下显著优于现有基线，具有高准确性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 长尾场景中类别样本量差异导致低频类别性能下降，基础模型的通用表示能力为解决此问题提供了潜力。

Method: 提出Sage方法，引入SG-Adapter将类别描述的语义信息融入视觉微调过程，并通过注意力机制增强视觉与文本模态的对齐；设计分布不匹配感知补偿因子纠正预测偏差。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频问答方法主要依赖事件中心的问题-答案对，缺乏对视频中对象类型、空间布局等基础细节的捕捉，导致模型学习到的场景表示不完整，限制了泛化和推理能力。

Method: FIQ通过生成基于视频描述的问题-答案对，丰富训练数据，并结合VQ-CAlign模块对齐任务特定问题嵌入与视觉特征，保留领域关键细节。

Result: 在SUTD-TrafficQA数据集上，FIQ实现了优于现有基线方法的性能。

Conclusion: FIQ通过增强对视频基础内容的理解，显著提升了视频问答模型的推理能力和泛化性。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本CIR方法存在信息丢失或视觉上下文利用不足的问题，需要一种更平衡的解决方案。

Method: MCoT-RE利用多角度思维链生成两个不同描述，结合多粒度重排序技术进行两阶段检索。

Result: 在FashionIQ和CIRR数据集上分别实现了6.24%和8.58%的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在组合图像检索中，早期融合忽视视觉上下文，晚期融合难以捕捉细粒度语义对齐，需要一种更优的融合策略。

Method: FAR-Net采用多阶段融合框架，包含增强语义对齐模块（ESAM，晚期融合）和自适应协调模块（ARM，早期融合），结合交叉注意力和不确定性嵌入。

Result: 在CIRR和FashionIQ数据集上，FAR-Net显著提升Recall@1和Recall@50，分别达到2.4%和1.04%的性能增益。
...
<details>
  <summary>Details</summary>
Motivation: 解决小目标多目标跟踪（SMOT）中因目标像素少导致的检测和外观关联不可靠问题。

Method: 引入SMOT4SB数据集，提出SO-HOTA评估指标，并举办MVA2025挑战赛。

Result: 数据集包含211个无人机视频序列，SO-HOTA指标优于IoU，竞赛中最佳方法性能提升5.1倍。

Conclusion: 为无人机场景下的SMOT研究奠定基础，适用于鸟击避免、农业等领域。
...
Conclusion: AnyCap项目通过模型、数据集和评估基准的整合，有效提升了可控字幕生成的性能。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像描述生成在环境监测、灾害评估等领域具有重要应用价值，但现有方法难以处理复杂卫星图像，因此需要更高效的解决方案。

Method: 提出了一种结合静态扩展、记忆增强自注意力和网格Transformer的Transformer网络架构。

Result: 在UCM-Caption和NWPU-Caption数据集上，所提模型在多数评估指标上优于现有最优方法。

Conclusion: 该模型展现了在实际遥感图像系统中应用的潜力。
...
Method: SRE通过模拟域偏移、注意力重聚焦和集成学习，减少域偏移并捕捉域不变注意力图。
...
Method: 提出SCORE框架，结合区域感知集成和全局上下文适应，分别优化类嵌入和文本嵌入，提升对象区分度和语言潜在空间的表达能力。

Result: 实验结果表明，SCORE在开放词汇遥感实例分割任务中达到SOTA性能，为大规模地理空间分析提供了稳健解决方案。

Conclusion: SCORE通过多粒度场景上下文整合，显著提升了遥感实例分割的开放词汇能力，适用于多样化地球观测场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉方法在光照差、遮挡和角度不佳时性能受限，Wi-Fi信号提供了一种替代方案。

Method: 从CSI提取生物特征，通过Transformer编码器和DNN处理，使用批内负损失函数训练。

Result: 在NTU-Fi数据集上取得与先进方法竞争的结果。

Conclusion: WhoFi通过Wi-Fi信号有效实现人员重识别，解决了视觉方法的局限性。
...
Method: 利用嵌入轻量项链式设备的软干电极，捕捉颈部组织阻抗变化；结合解剖学先验的深度学习框架估计头部姿态。

Result: 在7名参与者中测试，平均每顶点误差为25.9毫米，性能接近当前视觉方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖显式信号（如面部表情）或生理信号，但前者易被掩盖，后者设备复杂且影响自然行为。注视分析方法通常静态，未能捕捉动态交互。

Method: 利用标准高清摄像头无干扰捕捉用户眼部外观和头部运动，估计时空注视轨迹，建模注视行为的空间、语义和时间维度。

Result: 揭示了情绪是人与环境交互的复杂结果，而非单纯的生理反应，实现了用户无感知、实时、连续的情绪识别。

Conclusion: 该方法克服了现有技术的局限性，为情绪识别提供了更自然、通用且低成本的解决方案。
...
Result: 在OpenLane数据集上，LanePerf的MAE为0.117，Spearman相关系数为0.727，优于基线方法。

Conclusion: LanePerf为ADAS提供了高效的无标注性能评估方法，提升了在复杂驾驶场景中的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中数据隐私保护和算法性能的问题，并填补图像分类数据集的空白。
...
TL;DR: 论文介绍了AthleticsPose数据集，用于单目3D姿态估计，显著优于基线模型，并分析了其在实际运动分析中的潜力与局限。
...
Result: 模型在真实运动数据上表现优异，MPJPE降低75%，但对相机视角和主体尺度敏感。

Conclusion: 真实运动数据对模型性能至关重要，但模型在高速指标上存在偏差，需进一步改进。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法依赖3D点云，但点云重建常导致信息丢失，如纹理缺失或复杂结构失真。多视角图像能提供更详细的场景表示，弥补这些不足。

Method: Argus通过融合多视角图像和相机姿态生成场景特征，与3D特征交互，形成全面的3D感知嵌入。

Result: 实验表明，Argus在多种下游任务中优于现有3D-LMMs。

Conclusion: Argus通过多模态输入扩展LLMs的3D任务能力，有效提升场景理解的准确性和细节表现。
...
Result: 在W4A6和W4A8等低比特量化下表现优异，保持高质量图像生成和模型稳定性。

Conclusion: DMQ有效解决了扩散模型量化问题，尤其在低比特场景下表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 现有的可见光-红外行人重识别模型严重依赖标注的跨模态样本，但在实际应用中，跨模态身份标签往往不可获得。因此需要探索仅使用单模态样本身份标签的弱监督方法来解决这一问题。

Method: 提出异构专家协作一致性学习框架：1）利用各模态的标注数据独立训练专门的分类专家；2）让这些分类专家作为异构预测器，预测另一模态样本的身份；3）设计跨模态关系融合机制，有效整合不同专家的预测结果；4）在跨模态身份对应关系的隐式监督下，促进专家间的协作一致性学习。

Result: 在两个具有挑战性的数据集上进行实验验证，证明了所提方法的有效性，显著提升了模型提取模态不变特征的能力和跨模态身份识别性能。

Conclusion: 所提出的弱监督跨模态行人重识别方法能够在缺乏跨模态标签的情况下，通过异构专家协作学习有效建立跨模态身份对应关系，为实际应用中的跨模态行人重识别提供了可行的解决方案。
...
TL;DR: LoViC是一个基于扩散变换器（DiT）的框架，通过分段生成过程生成长且连贯的视频，解决了长视频生成中的复杂性问题。
...
Conclusion: LoViC通过创新的分段生成和统一的潜在表示，成功解决了长视频生成的挑战，同时保持了时间连贯性和可扩展性。
...
TL;DR: 提出了一种基于隐式神经表示（INRs）的新型可变形图像配准（DIR）框架cIDIR，通过条件化正则化超参数，避免了传统方法需要多次训练的缺点。


<details>
  <summary>Details</summary>
Motivation: 传统DIR框架中正则化参数调优计算成本高，需多次训练，cIDIR旨在解决这一问题。

Method: cIDIR基于INRs，通过正则化超参数的先验分布进行训练，并利用分割掩模优化超参数。

Result: 在DIR-LAB数据集上，cIDIR表现出高精度和鲁棒性。

Conclusion: cIDIR通过条件化超参数和连续可微的DVF，实现了高效且灵活的图像配准。
...
<details>
  <summary>Details</summary>
Motivation: 手腕病理诊断耗时且需专业知识，计算机视觉虽具潜力，但医疗影像数据稀缺，单一模态（如图像）效果有限。

Method: 1. 将问题视为细粒度识别任务；2. 融合患者元数据与X光图像；3. 使用细粒度数据集预训练权重。

Result: 细粒度策略和元数据整合使诊断准确率在小数据集上提升2%，在大骨折数据集上提升超10%。

Conclusion: 多模态和细粒度方法可有效提升有限数据集下的手腕病理识别性能。
...
TL;DR: 该论文提出了一种基于RGB预训练潜在扩散模型（RGB-LDM）的扩展方法，用于从RGB图像重建高光谱图像（HSI），通过分离不可观测特征并学习其联合分布，实现了卓越的性能。
...
Method: 提出两阶段流程：1）通过光谱不可观测特征自编码器（SpeUAE）提取和压缩不可观测特征；2）利用RGB-LDM扩展为不可观测特征LDM（ULDM），学习联合分布。
...
<details>
  <summary>Details</summary>
Motivation: 大型模型（如Vision Transformers）训练成本高，部署时面临延迟、计算和内存问题，现有结构化剪枝方法需要昂贵重训练。

Method: 通过收集激活统计数据选择剪枝神经元，同时将平均激活集成回模型以保持性能。

Result: 在ImageNet-1k任务中，剪枝后DeiT-Base保留70%性能，仅需10轮微调恢复99%准确率，减少35%计算和36%模型大小，速度提升1.44倍。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有弱监督方法因标注信号不足导致的性能限制问题，提高标注效率和检测性能。

Method: 采用涂鸦标注重新标注数据集，提出自监督训练、先验感知特征调制模块（PFMM）、门控自适应融合模块（GAFM）和置信感知熵最小化损失（${\mathcal{L}}_{ {CEM }}$）。

Result: 实验表明，该方法在分布内和分布外数据上均优于现有全监督方法。

Conclusion: 涂鸦标注结合提出的模块和损失函数，显著提升了弱监督图像篡改定位的性能。
...
Method: 评估图像tokenizer，改进Bidirectional LLaMA架构，引入双向注意力和2D RoPE，形成MaskGIL模型。

Result: MaskGIL在ImageNet 256x256基准测试中FID达3.71，仅需8步推理，性能媲美AR模型。

Conclusion: MaskGIL不仅提升图像生成效率，还扩展至文本驱动生成和实时语音转图像应用。
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键的机器人应用（如自动驾驶）中，实时准确地检测动态对象至关重要。现有方法因使用多个子网络而计算成本高，影响实时性。
...
Conclusion: Decoupled PROB通过解耦对象性与类别预测，有效解决了PROB的问题，为OWOD任务提供了更优的解决方案。
...
Method: 结合LoRA和梯度正则化技术，优化模型参数，提高稳定性。
...
TL;DR: 论文提出了一种基于深度学习的自动化胎儿肺成熟度评估流程，通过3D nnU-Net模型实现肺部分割，并结合模型拟合评估成熟度，结果显示自动化与手动方法无显著差异。


<details>
  <summary>Details</summary>
Motivation: 胎儿肺成熟度是预测新生儿结局的关键指标，但目前依赖手动分割的方法耗时且限制临床应用。

Method: 使用3D nnU-Net模型对扩散加权MRI图像进行自动肺部分割，并结合模型拟合评估成熟度。

Result: 分割模型平均Dice系数为82.14%，自动化与手动方法在IVIM参数上无差异。
...
TL;DR: 提出了R²MoE框架，通过路由蒸馏和冗余专家移除技术，解决了持续学习中的灾难性遗忘和参数膨胀问题，显著提升了概念保真度并减少了参数数量。


<details>
  <summary>Details</summary>
Motivation: 解决大规模生成模型在持续学习新视觉概念时面临的灾难性遗忘和参数膨胀问题。

Method: 采用混合专家框架，结合路由蒸馏机制、冗余专家移除策略和分层局部注意力引导推理。

Result: 在CustomConcept 101数据集上，遗忘率降低87.8%，参数减少63.3%，生成图像概念保真度优于现有方法。

Conclusion: R²MoE是一种参数高效的持续学习框架，有效解决了灾难性遗忘和参数膨胀问题，适用于个性化生成模型。
...
Method: 通过将测试样本配准到多个正常原型以直接比较结构，采用关键点引导的聚类策略进行局部异常评估，计算测试样本与原型在聚类内的特征相似性。

Result: 在Real3D-AD基准测试中，该方法在对象级和点级异常检测上均达到最先进性能，仅使用原始特征。

Conclusion: 提出的框架通过多原型对齐和关键点引导的聚类，实现了高精度3D异常定位，解决了点云数据的挑战。
...
TL;DR: 提出了一种结合语言先验的多模态红外小目标检测框架，利用文本信息提升检测能力，并在新构建的数据集上验证了其优越性。
...
Method: 利用GPT-4生成目标位置的文本描述，通过语言引导的注意力权重增强检测能力，构建多模态数据集。

Result: 在NUAA-SIRST和IRSTD-1k子集上，IoU、nIoU、Pd和Fa指标显著优于现有方法。

Conclusion: 多模态框架结合语言先验有效提升了红外小目标检测性能，为未来研究提供了新方向。
...
Method: 提出RS-TinyNet，包含微小物体显著性建模和特征完整性重建两个核心设计，以及三个逐步特征增强模块（MDCA、ARB和PFDH）。
...
<details>
  <summary>Details</summary>
Motivation: 学习型单目视觉里程计在机器人领域面临鲁棒性、泛化性和效率的挑战，而现有视觉基础模型（如DINOv2）的特征粒度较粗，限制了其在VO中的应用。
...
Conclusion: DINO-VO通过结合语义与几何特征，显著提升了单目视觉里程计的鲁棒性和泛化能力，同时保持了高效性。
...
TL;DR: 提出了一种自进化的视觉语言导航框架（SE-VLN），通过结合记忆模块、检索增强推理模块和反思模块，实现了导航性能的持续提升。
...
Method: SE-VLN包含三个核心模块：分层记忆模块、检索增强推理模块和反思模块，支持知识复用和多步决策。
...
TL;DR: 该论文提出了一种用于自动驾驶的世界模型，通过简单设计实现了长时程生成和复杂场景泛化，无需额外监督或传感器。模型在困难场景中表现优异，并通过对比实验支持连续自回归模型的优势。


<details>
  <summary>Details</summary>
Motivation: 解决现有自动驾驶世界模型在长时程生成和复杂场景泛化上的不足，探索离散与连续模型的性能差异。

Method: 采用简单设计，无需额外监督或传感器，构建混合分词器以对比离散与连续模型。

Result: 模型在困难场景（如转弯和城市交通）中表现优异，参数较少（469M）且训练数据量较小（280h视频）。
...
TL;DR: 该研究提出了一种利用生成式AI平台Midjourney为建筑工人检测任务生成合成图像的方法，解决了数据多样性和数量不足的问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习在视觉AI中的应用受限于建筑领域数据稀缺，需要一种方法生成多样且真实的训练数据。

Method: 通过3000种不同提示生成12000张合成图像，并手动标注，用于训练DNN。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频质量的提升，检测此类内容对打击虚假信息、保护隐私和安全至关重要。现有方法主要针对DeepFakes，而通用视频检测需求迫切。
...
<details>
  <summary>Details</summary>
Motivation: 追求一个无需数据集特定微调、能在不同分辨率和视差范围内工作的泛化立体匹配模型，揭示了局部搜索方法与全局匹配架构之间的根本性权衡。
...
Result: VITA在ALOHA平台的双臂操作任务中表现优异，推理延迟降低50-130%，性能优于或匹配现有生成策略。

Conclusion: VITA是首个仅用MLP层解决复杂双臂操作任务的流匹配策略，展示了高效和简化的优势。
...
<details>
  <summary>Details</summary>
Motivation: 观察到预训练ViT的权重矩阵具有近似正交性，而低秩适应矩阵（如LoRA和Adapter）缺乏这一特性，可能导致泛化能力不足。因此，研究如何通过正交性提升微调模型的泛化能力。

Method: 提出AOFT策略，使用单个可学习向量生成近似正交向量，构建低秩适应矩阵，使其与预训练权重矩阵的正交性对齐。
...
TL;DR: DiffClean通过文本引导扩散模型消除化妆痕迹，提升年龄验证和面部识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 在线平台需准确验证年龄以保护未成年用户，但化妆可能干扰年龄估计。
...
Result: 实验展示了细粒度姿势合成和高效一致的重光照效果。
...
Method: 通过细致的视觉匹配改进基准的真实标注，并公开这些标注。

Result: 改进了基准的真实标注，提升了评估的可靠性和可重复性。

Conclusion: 改进后的标注资源将促进姿态感知多模态推理领域的进步。
...
Conclusion: 该方法在实时性和准确性上均表现良好，适用于流式自我中心视觉应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决DiT在资源受限设备上计算成本高的问题，实现移动平台实时视频生成。

Method: 采用压缩VAE降维、KD引导的三层次剪枝和对抗步蒸馏技术。

Result: 在iPhone 16 Pro Max上实现10+FPS的实时生成。

Conclusion: 优化后的DiT模型在移动设备上实现高质量实时视频生成。
...
Result: 在新提出的复杂概念基准Inert-CompBench和两个公开测试集上，显著提升了基线模型的概念响应能力，效果具有竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D对象部件生成任务中组合性和自动化的需求，利用潜在空间的几何表达能力。

Method: 采用自回归方式逐步生成部件，基于输入条件（如2D图像或3D对象）和已生成部件，自动确定部件类型和数量。
...
<details>
  <summary>Details</summary>
Motivation: 分层流匹配虽然能建模多模态速度分布，但各层级分布复杂性相同，限制了模型表现。

Method: 提出使用小批量耦合逐步调整分层流匹配中不同层级的分布复杂性。
...
TL;DR: VideoITG通过指令引导的帧选择和VidThinker管道提升视频大语言模型的性能，构建了VideoITG-40K数据集，并在多模态视频理解任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前方法在长视频理解中难以处理复杂场景，需要更有效的帧选择方法。

Method: 提出VideoITG，基于用户指令定制帧采样，通过VidThinker管道生成详细注释并选择关键帧。

Result: 构建了VideoITG-40K数据集，模型在多模态视频理解任务中表现显著提升。
...
Result: 系统支持学生模块化学习、针对性复习和个性化练习，提升数学学习效果。
...
TL;DR: 论文提出了一种基于博弈论的高速公路并道场景模拟模型，改进了战术决策的收益函数和滞后动作，结合动力学模型实现更真实、可解释的交互模拟。


<details>
  <summary>Details</summary>
Motivation: 提升模拟环境中驾驶员行为的真实性，以支持自动驾驶技术的发展，特别是在高速公路并道场景中。

Method: 采用博弈论模型改进战术决策的收益函数和滞后动作，结合动力学模型实现统一决策与动力学模拟。

Result: 模型在真实数据集上验证了复杂交互的可重现性，并在高保真模拟环境中展示了计算效率。

Conclusion: 该模型为自动驾驶开发提供了可扩展且高效的模拟工具，能够更真实地模拟并道交互。
...
Method: 提出了一种基于“What”（解释目标）和“How”（解释方式）的分类法，并用此对XRL领域的250多篇论文进行了系统综述。

Result: 通过分类法梳理了XRL的研究现状，并指出了该领域需要关注的邻近领域和未来需求。

Conclusion: XRL领域需要进一步研究以解决其挑战，并扩展与其他领域的交叉研究。
...
TL;DR: 论文提出了一种结合强化学习（RL）代理和多模态模型（LMM）的自动化游戏设计迭代框架，通过分析玩家行为动态优化游戏机制。


<details>
  <summary>Details</summary>
Motivation: 现代生成系统难以从静态规则和内容中捕捉动态玩家行为，因此需要一种方法填补这一差距。

Method: 框架通过RL代理进行游戏测试，生成数值指标或图像摘要，再由LMM分析并修改游戏配置以实现目标。
...
TL;DR: 论文探讨了AI助手可能欺骗用户的问题，研究了白盒和黑盒监控在检测欺骗中的效果，发现现有欺骗探针的效果较弱但有潜力。


<details>
  <summary>Details</summary>
Motivation: AI助手偶尔会欺骗用户，现有欺骗探针的效果和抗干扰能力尚不明确。

Method: 比较白盒监控（可访问令牌级探针激活）和黑盒监控（无此访问权限），通过黑盒到白盒的性能提升评估欺骗探针效果。

Result: 发现现有欺骗探针的黑盒到白盒性能提升较弱，但结果令人鼓舞。

Conclusion: 现有欺骗探针在检测欺骗方面有一定潜力，但效果仍需改进。
...
<details>
  <summary>Details</summary>
Motivation: 同伴学习虽有效但受限于同伴水平，AI伴侣可解决这一问题。
...
TL;DR: 本文介绍了在NLPCC 2025 Task 8中，利用基于提示工程和微调技术的大规模语言模型，提升情感支持对话（ESC）任务的表现。最佳模型在比赛中排名第二。
...
Result: 最佳模型在比赛中排名第二，验证了大规模语言模型结合有效适应方法的潜力。
...
TL;DR: 论文探讨了人类智能快速适应新环境的能力与高效构建和优化世界模型的关系，提出了一种新的评估框架，用于测试AI系统的世界模型适应性。
...
Method: 借鉴认知科学研究，提出基于新颖游戏套件的评估范式，设计具有持续新颖性的游戏来测试AI的世界模型构建能力。

Result: 提出了一种新的评估框架和指标，旨在挑战和评估AI在快速构建世界模型方面的能力。
...
TL;DR: 论文提出了一种方法，将人类判断从模拟决策循环中移出，设计伦理度量空间供模拟环境探索，最后返回少数选项供人类指挥官选择。
...
Method: 人类设计伦理度量空间，模拟环境探索空间并返回选项，人类指挥官从中选择最优方案。研究重点是如何动态加权伦理决策中的属性。
...
<details>
  <summary>Details</summary>
Motivation: 前沿AI系统在说服、欺骗和影响人类行为方面的能力迅速提升，可能威胁到人类监督和网络安全，但目前缺乏系统性框架来应对这一风险。

Method: 提出了一个安全案例框架，围绕无能性、控制性和可信性三个核心论点，详细说明了证据要求、评估方法和实施考虑。

Result: 为AI公司提供了一种系统性方法，将操纵风险纳入AI安全治理，为其评估和缓解威胁提供了具体基础。
...
Method: 引入VAR-MATH框架，将固定数值问题转化为符号模板，要求模型解决多个变体，以评估一致性推理能力。
...
<details>
  <summary>Details</summary>
Motivation: PEC在不确定环境中具有强大的叙事推理能力，但缺乏目标导向推理机制，因此需要将其与MDP结合以扩展功能。

Method: 通过引入“动作执行情境”概念，将PEC领域形式化转化为MDP，并保留PEC的灵活动作语义。

Result: PEC-MDP形式化支持时间推理和目标驱动规划，并能将学习到的策略映射回可读的PEC表示。

Conclusion: PEC-MDP结合了PEC的可解释性和MDP的算法工具，扩展了PEC的功能，同时保持其叙事推理优势。
...
TL;DR: X-MILP是一种基于约束推理技术的领域无关方法，用于为混合整数线性规划（MILP）提供对比解释，通过构建“原因图”帮助用户理解查询答案的结构。
...
Method: 将用户查询编码为额外约束，计算不可约不可行子系统（IIS），并构建“原因图”来表示解释。

Result: 在经典优化问题上测试，验证了计算解释的实用性。

Conclusion: X-MILP提供了一种有效的对比解释方法，适用于MILP问题。
...
Method: 使用加州78号公路5个月的30秒间隔数据，采用MLR和RF算法分析不同数据收集间隔（30秒至15分钟）。
...
<details>
  <summary>Details</summary>
Motivation: 现有树结构推理方法（如ProbTree）存在静态树结构和计算效率低的问题，无法动态适应中间结果。
...
TL;DR: 论文提出修订后的十项功能标准，用于评估基于大型语言模型（LLM）的人工道德代理（AMA），以应对LLM不透明性带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 由于LLM的随机输出和不透明内部状态，传统伦理标准已不适用，需要新的评估框架。

Method: 提出十项功能标准，并通过模拟道德代理（SMA-LLS）和自动驾驶公交车（APB）案例进行验证。

Result: 修订后的标准（如道德一致性、上下文敏感性等）能更好地指导LLM的道德代理设计。

Conclusion: 新标准有助于LLM在道德代理中实现更好的社会整合与对齐。
...
<details>
  <summary>Details</summary>
Motivation: 在涉及抽象函数和谓词推理的决策任务中，高阶理论与模糊逻辑的结合非常有用，但开发高效的计算方法是一个挑战。

Method: 采用高阶模式和基于最小T-范数的模糊等价关系，提出统一算法。

Result: 算法能计算最一般的统一器，并在可统一时提供最高近似度。

Conclusion: 该算法解决了高阶模式与模糊等价关系的统一问题，具有理论意义和实用价值。
...
TL;DR: 论文提出了一种名为GEA（Generative Energy Arena）的新方法，通过公开竞技场评估大语言模型，并引入能源消耗信息，初步结果显示用户倾向于选择更节能的模型。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估方法（如自动化基准测试或人工评估）存在局限性，如与人类评价相关性低或成本高。因此，需要一种更高效且能反映用户偏好的评估方法。

Method: 提出GEA，一个公开竞技场，用户可以在评估模型时看到能源消耗信息，从而选择更节能的模型。

Result: 初步结果显示，用户在了解能源消耗后，倾向于选择更小、更节能的模型，而非性能更高的复杂模型。

Conclusion: GEA提供了一种新的评估方法，表明用户更关注能源效率而非模型性能，为未来模型设计提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 探索人类如何利用广泛背景知识进行推理，并提出一种计算模型来模拟这一能力。

Method: 结合语言模型实现全局相关性检索与模型合成，使用概率程序构建定制化世界模型。

Result: MSA在模拟人类判断上优于纯语言模型，尤其在处理新颖因果结构和背景知识时表现更佳。
...
TL;DR: 该论文研究了语言模型（LMs）如何区分句子的模态类别（如可能、不可能或无意义），并发现了一种称为模态差异向量的线性表示方法，揭示了LMs在模态分类上的能力优于以往研究。此外，模态差异向量的出现与模型能力的提升一致，并能模拟人类细粒度的分类行为。


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型广泛应用于多种任务，但其在模态分类上的能力受到质疑。本研究旨在验证LMs是否具备可靠的模态分类能力，并探索其与人类分类行为的关联。

Method: 通过识别LMs中的模态差异向量，分析其在模型训练过程中的出现顺序，并将其与人类分类行为进行对比。

Result: 研究发现LMs的模态分类能力优于以往报告，模态差异向量与模型能力提升一致，并能模拟人类分类行为。
...
Method: 通过微调NLLB-200多语言翻译模型，加入车臣语支持。

Result: 俄语到车臣语的BLEU/ChrF++得分为8.34/34.69，反向为20.89/44.55。

Conclusion: 发布翻译模型及相关语料库，推动车臣语的技术应用。
...
TL;DR: 该论文提出使用自然语言处理（NLP）模型，特别是经过微调的BioClinicalBERT，来自动分类药物过量死亡的自由文本报告，显著提高了准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 美国药物过量死亡率的上升，尤其是芬太尼相关死亡，需要及时准确的监测。然而，关键数据通常隐藏在自由文本的验尸报告中，手动编码为ICD-10分类会导致延迟和信息丢失。

Method: 研究使用了35,433份2020年的死亡记录进行模型训练和内部测试，并采用3,335份2023-2024年的新数据集进行外部验证。评估了多种NLP方法，包括传统分类器、微调的BERT和BioClinicalBERT模型，以及当代大型语言模型如Qwen 3和Llama 3。

Result: 微调的BioClinicalBERT模型在内部测试集上表现近乎完美（宏F1分数≥0.998），外部验证也证实其稳健性（宏F1=0.966），优于传统机器学习和其他模型。

Conclusion: NLP模型，尤其是BioClinicalBERT，为药物过量死亡分类提供了高精度、可扩展的解决方案，能够显著加速监测流程，支持实时检测新兴药物使用趋势。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在处理多模态数据时未能充分捕捉文本与视觉信息的交互关系，AdaptiSent旨在解决这一问题。

Method: 采用动态模态加权和上下文自适应注意力机制，优化文本与图像信息的交互提取。

Result: 在标准Twitter数据集上，AdaptiSent在精度、召回率和F1分数上超越现有模型，尤其在复杂多模态关系识别上表现突出。

Conclusion: AdaptiSent为多模态方面情感分析设定了新标准，显著优于现有方法。
...
TL;DR: 论文提出AudioJudge，利用大型音频模型（LAM）作为统一评估框架，解决现有语音评估中针对特定音频特性设计系统的困难及自动评估与人类偏好相关性差的问题。


<details>
  <summary>Details</summary>
Motivation: 当前语音评估存在两个关键问题：一是需要针对不同音频特性设计专门系统，难度大；二是自动评估方法与人类偏好相关性差。

Method: 通过系统研究LAM作为评估工具（AudioJudge），探索其在音频特性检测（如发音、语速、说话人识别和语音质量）和系统级人类偏好模拟任务中的应用。采用音频拼接和上下文学习策略提升性能，并引入多角度集成方法。

Result: AudioJudge在音频特性检测和人类偏好模拟任务中表现优异，多角度集成方法在系统排名基准中与人类偏好的Spearman相关性达到0.91。

Conclusion: LAM作为统一评估框架具有潜力，但需注意其存在的冗长性和位置偏差问题。
...
TL;DR: 论文提出FLEXITOKENS，一种可学习的分词器，通过简化训练目标提高语言模型在新数据分布下的适应性，减少分词过碎问题。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型的子词分词器在适应新数据分布时表现僵化，导致分词效率低下，特别是在未见过的语言或脚本中。

Method: 开发了基于字节的语言模型，包含可学习的分词器子模块，预测输入字节序列的边界，形成可变长度分段。提出FLEXITOKENS训练目标，避免固定压缩率的限制。

Result: 在多语言基准测试、形态多样性任务和不同领域中，FLEXITOKENS显著减少分词过碎，下游任务性能提升高达10%。

Conclusion: FLEXITOKENS通过灵活的分词器设计，有效解决了传统分词器的僵化问题，提升了语言模型的适应性。
...
Result: 在多种语言对上表现优于MT-Ranker，评估结果与人类评分高度一致。
...
TL;DR: 提出了一种无需额外训练的解码方法ThinkLogit，通过小模型引导大模型进行长链推理，并结合偏好优化进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何通过模型自动学习不同抽象层次的语言表示，减少对外部分词器的依赖，提高模型的鲁棒性和灵活性。

Method: 训练一个字节级语言模型，通过学习路由机制实现不同抽象层次的桥接，并与BBPE和Llama3进行比较。

Result: Synergy在相同模型规模和训练数据量下表现优于Llama3，且中间部分在去除位置编码时表现更好，表明位置无关概念的出现。
...
TL;DR: 提出了一种通过从大型语言模型中蒸馏数据来提升文本编码器否定鲁棒性的方法，显著改善了否定理解能力。
...
Method: 利用大型语言模型生成多样化的否定和模糊表达数据，采用对比学习微调BERT模型。

Result: 显著提升了否定理解能力，同时在通用基准上保持竞争力；方法也可适配大语言模型，提升否定基准表现。
...
TL;DR: 论文研究了大型语言模型（LLM）如何通过文本提示生成符号音乐数据，并评估其在音乐识别和生成任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM是否隐含地建模符号音乐，填补这一领域的研究空白。

Method: 通过文本提示生成MIDI文件，训练神经网络进行音乐分类和旋律补全任务。

Result: LLM能够推断基本的音乐结构和时间关系，但缺乏明确的音乐上下文限制了其表现。

Conclusion: LLM在符号音乐生成中具有潜力，但也存在局限性。
...
TL;DR: 论文探讨了跨语言一致性对多语言模型知识传递的重要性，分析了知识一致性的表现及影响因素，并评估了提升一致性的策略。


<details>
  <summary>Details</summary>
Motivation: 研究跨语言一致性对评估多语言模型知识传递、保持事实性及性能一致的重要性。

Method: 通过分析代码混合的共指陈述，使用可解释性方法研究模型行为，并评估提升一致性的策略。

Result: 多语言模型一致性受语言家族、语言因素及特定层瓶颈影响；代码切换训练和跨语言词对齐策略效果最佳。
...
Conclusion: 研究表明可以从零开始预训练通用的分层推理器。
...
Method: 使用开源LLM生成自然语言指令并转换为Python代码，通过多步骤流程（分析表格、选择列、生成指令、代码转换、运行及错误处理）完成任务。

Result: 在任务中实现了85%的准确率。

Conclusion: 该方法通过LLM生成代码的多步骤流程，有效提升了表格问答任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 自我是一个多面构造，在语言中有所体现，但在自然语言处理（NLP）中尚未充分探索。其与心理健康等现象相关，需要系统化的NLP分析。

Method: 计划构建自我方面的本体和标注数据集，开发判别模型、生成模型和嵌入检索方法，评估其可解释性、准确性等标准。

Result: 最佳模型将用于心理健康和现象学的案例研究。

Conclusion: 该框架有望为NLP中的自我分析提供系统化工具，并推动相关应用领域的研究。
...
TL;DR: 研究探讨了标注者人口统计特征对标注决策的影响，发现其仅占8%的方差，推文内容是主要因素。评估生成式AI作为标注者的可靠性，发现简单的人口统计提示通常无效甚至有害。
...
Result: 人口统计因素仅占8%的方差，推文内容是主导因素；生成式AI的简单提示通常无效。
...
TL;DR: 研究通过机器学习分析儿童口语叙事特征，发现词汇多样性和句子长度是典型发展的指标，而特定动词和助动词的使用与减少干预需求相关。


<details>
  <summary>Details</summary>
Motivation: 探索儿童口语叙事能力对后期读写发展的预测作用，特别是在多语言环境中。
...
TL;DR: GEMMAS是一个基于图的评估框架，通过建模多智能体交互为有向无环图，分析协作过程，并提出两个过程级指标（IDS和UPR）以评估协作质量。
...
Method: 引入GEMMAS框架，建模智能体交互为有向无环图，并提出IDS和UPR指标。
...
<details>
  <summary>Details</summary>
Motivation: 早期叙事和理解能力对后期读写能力至关重要，但教师在大班教学中难以准确识别需要干预的学生。

Method: 系统结合自动语音识别和机器学习评分模型（线性模型与大型语言模型LLM对比），预测叙事和理解分数。

Result: LLM模型在多数情况下优于线性模型，且与人类专家在识别需要干预的学生上表现相当。
...
TL;DR: 论文提出了一种名为CAST的框架，通过操纵大语言模型（LLMs）的内部激活状态，实现跨任务知识迁移，无需参数更新或输入扩展。
...
Method: 分析LLMs潜在空间中的激活模式，提出CAST框架，通过对比表示增强的激活状态实现跨任务迁移。

Result: 在跨领域和跨语言迁移实验中，CAST表现优于基线方法，具有更高的可扩展性和更低的计算成本。

Conclusion: CAST通过潜在空间操控实现了高效的跨任务知识迁移，为LLMs在数据稀缺场景下的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注英语环境下的大模型推理能力，而对印地语等印度语言的研究不足，限制了模型跨语言泛化能力的理解。
...
Conclusion: HATS填补了印地语推理评估资源的空白，提出的方法为提升模型跨语言推理能力提供了有效途径。
...
TL;DR: 论文提出了一种名为QuestA的简单有效策略，通过问题增强（引入部分解决方案）提升强化学习在数学推理任务中的表现，显著提高了模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在训练大型语言推理模型中起重要作用，但其在多步推理（尤其是难题）上的效果受到质疑。为了解决这一问题，作者提出了QuestA方法。

Method: QuestA通过在训练中引入部分解决方案来降低问题难度并提供更有信息量的学习信号，从而提升强化学习的效果。

Result: 实验表明，QuestA在数学推理任务中显著提高了pass@1和pass@k指标，并在多个基准测试中取得了新的最优结果（如AIME24、AIME25和HMMT25）。

Conclusion: QuestA不仅提升了推理能力，还提高了样本效率，为通过强化学习扩展推理能力提供了实用且可推广的途径。
...
<details>
  <summary>Details</summary>
Motivation: 解决人力资源管理领域缺乏公开数据和基准的问题，促进可靠、公平的语言模型发展。

Method: 通过构建真实、多样化的数据集，设计多语言任务，并评估性别偏见，吸引团队参与。

Result: 76个团队参与，提交280多次，结果显示训练策略比模型规模更重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法生成的演示存在逻辑不一致和布局不佳的问题，难以达到专业标准，因此需要一种更高质量的自动化解决方案。

Method: RCPS整合了深度结构化叙事规划、自适应布局生成和迭代优化循环，同时提出PREVAL评估框架进行多维度质量评估。

Result: 实验表明，RCPS在所有质量维度上显著优于基线方法，PREVAL与人类评估结果高度相关。

Conclusion: RCPS能够生成接近人类专家水平的演示，PREVAL为自动化评估提供了可靠工具。
...
TL;DR: AbGen是首个评估LLMs在设计科学研究的消融研究能力的基准，包含1,500个专家标注的NLP论文示例。评估显示LLMs与人类专家在重要性、忠实性和合理性上存在显著差距，且当前自动评估方法不可靠。为此开发了AbGen-Eval基准，用于评估自动评估系统的可靠性。
...
Method: 基于1,500个专家标注的NLP论文示例，要求LLMs生成消融研究设计，并评估其性能。开发AbGen-Eval基准以评估自动评估系统的可靠性。
...
Method: 创建HapticCap数据集，提出触觉-描述检索任务，使用T5语言模型和AST音频模型的结合进行对比学习。

Result: T5和AST结合在触觉-描述检索任务中表现最佳，尤其是分类别训练时。
...
TL;DR: VL训练对语言模型的词汇-概念知识部署有影响，但对其分类知识本身改变不大。


<details>
  <summary>Details</summary>
Motivation: 探讨视觉-语言（VL）训练是否显著改变语言模型的词汇-概念知识，尤其是其分类组织。

Method: 通过比较纯文本语言模型和VL训练模型在文本问答任务中的表现，并进行行为和分析。

Result: VL模型在需要分类理解的问答任务中表现更好，但分类知识本身未显著改变。

Conclusion: VL训练改善了分类知识在任务中的部署，而非知识本身。
...
TL;DR: 本文提出了一种名为TAIL的方法，通过模仿图灵机的执行过程生成链式思维数据，以提升大型语言模型在长度泛化上的能力。实验表明，TAIL显著提升了模型性能。
...
TL;DR: 该论文提出了“上下文工程”这一系统性优化大语言模型（LLM）信息负载的学科，并对其基础组件和系统实现进行了分类。研究发现，当前模型在理解复杂上下文方面表现出色，但在生成高质量长文本方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于通过系统化优化LLM的上下文信息负载，提升其性能，并揭示当前模型在生成能力上的局限性。

Method: 方法包括对1300多篇研究论文的系统分析，提出了上下文工程的基础组件（如上下文检索与生成、处理与管理）和系统实现（如检索增强生成、记忆系统、多智能体系统）。

Result: 研究结果表明，当前模型在理解复杂上下文方面表现优异，但在生成高质量长文本方面存在显著不足。

Conclusion: 结论指出，未来研究需解决模型在生成能力上的局限性，并为上下文感知AI的发展提供了统一框架。
...
TL;DR: 研究探讨大型语言模型（LLMs）解释幽默的能力是否依赖幽默形式，发现现有模型无法可靠解释所有笑话类型。


<details>
  <summary>Details</summary>
Motivation: 现有计算幽默研究多集中于简单双关笑话，而忽略了更复杂的幽默形式，如需要世界知识的时事或流行文化笑话。
...
TL;DR: 论文探讨了联邦学习（FL）在基于摄像头的道路状况分类（RCC）系统中的隐私保护潜力，但揭示了其易受目标标签翻转攻击（TLFAs）的漏洞，并提出了一种防御机制FLARE。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决FL-RCC系统中恶意客户端通过篡改标签数据破坏模型性能的安全问题。

Method: 论文提出了一种基于标签距离的度量标准来量化TLFAs风险，并设计了FLARE防御机制，通过输出层的神经元分析来减轻攻击影响。
...
### [147] [Prompt Injection 2.0: Hybrid AI Threats](https://arxiv.org/abs/2507.13169)
*Jeremy McHugh,Kristina Šekrst,Jon Cefalu*

Main category: cs.CR

TL;DR: 论文分析了Prompt Injection 2.0攻击，探讨其如何结合传统网络安全漏洞（如XSS、CSRF）绕过传统安全措施，并提出结合提示隔离、运行时安全和特权分离的解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理系统的普及，Prompt Injection攻击演变为更复杂的混合威胁，传统安全措施已无法应对，亟需新的防护方法。

Method: 基于Preamble的研究，评估现有缓解技术对现代威胁（如AI蠕虫、多代理感染）的效果，并提出结合多种安全措施的架构解决方案。

Result: 传统安全措施（如WAF、XSS过滤器）对AI增强攻击无效，新提出的架构方案能有效检测和防御混合威胁。

Conclusion: Prompt Injection 2.0威胁严峻，需结合新型安全技术与传统措施以应对AI增强的混合攻击。
...
### [148] [On multiagent online problems with predictions](https://arxiv.org/abs/2507.12486)
*Gabriel Istrate,Cosmin Bonchis,Victor Bogdan*

Main category: cs.MA

TL;DR: 研究了在多智能体环境中使用预测的竞争算法的性能，提出了一个双预测框架，分析了在不同预测质量假设下的最佳竞争比。
...
Result: 在完美预测其他智能体行为时，遵循自身预测的算法是最优的，但对自身行为预测错误不鲁棒；提出了一种更具鲁棒性的算法。

Conclusion: 双预测框架在多智能体环境中具有潜力，但需要平衡预测准确性和鲁棒性。
...
### [149] [Achieving Robust Channel Estimation Neural Networks by Designed Training Data](https://arxiv.org/abs/2507.12630)
*Dianxin Luan,John Thompson*

Main category: eess.SP

TL;DR: 论文提出了一种离线训练的神经网络设计标准，用于生成合成训练数据集，确保网络在新信道上达到特定MSE，无需实时训练或信道信息。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法在新数据上表现不佳，且物理信道通常是时变的，但低延迟和有限计算资源限制了在线训练。

Method: 提出设计标准生成合成训练数据集，确保神经网络在新信道上达到特定MSE，并进一步提出基准设计。

Result: 神经网络在不同信道配置下表现出稳健的泛化能力，且与网络架构无关。

Conclusion: 离线训练的神经网络设计标准可实现无需实时调整的稳健信道估计。
...
### [150] [Autonomy for Older Adult-Agent Interaction](https://arxiv.org/abs/2507.12767)
*Jiaxin An*

Main category: cs.HC

TL;DR: 本文探讨了AI代理在老年人护理中的自主性问题，提出了四个自主性维度，并建议了三个研究方向。
...
Method: 通过跨学科视角定义老年人的四个自主性维度（决策自主性、目标导向自主性、控制自主性和社会责任自主性），并提出了三个研究方向。

Result: 提出了社会责任自主性的重要性、从任务角度操作化代理自主性以及开发自主性测量工具的研究方向。

Conclusion: 本文为AI代理在老年人护理中的自主性问题提供了理论框架和研究方向，强调了社会责任和伦理问题的重要性。
...
### [151] [Catching Dark Signals in Algorithms: Unveiling Audiovisual and Thematic Markers of Unsafe Content Recommended for Children and Teenagers](https://arxiv.org/abs/2507.12571)
...
TL;DR: 研究分析了短视频平台对儿童和青少年的潜在危害，发现不安全视频具有暗色视觉特征和显性或隐性有害内容，提出了在线危害框架，呼吁加强内容审核和平台监管。


<details>
  <summary>Details</summary>
Motivation: 短视频平台的普及和年龄验证机制的失效引发了对儿童和青少年在算法调节的在线环境中潜在危害的关注。

Method: 通过对Instagram Reels、TikTok和YouTube Shorts上推荐的4,492个短视频进行多模态特征分析和主题建模。

Result: 不安全视频具有暗色视觉特征，并包含显性有害内容和隐性焦虑诱导内容。

Conclusion: 研究强调保护儿童和青少年免受显性和隐性风险的重要性，呼吁更细致的内容审核和平台监管。
...
### [152] [ParaStudent: Generating and Evaluating Realistic Student Code by Teaching LLMs to Struggle](https://arxiv.org/abs/2507.12674)
*Mihran Miroyan,Rose Niousha,Joseph E. Gonzalez,Gireeja Ranade,Narges Norouzi*

Main category: cs.CY

TL;DR: ParaStudent研究探讨了LLM是否能生成类似学生的不完美、迭代且风格多样的代码，并通过实验证明微调能显著提升模型对学生学习轨迹的模拟能力。
...
Method: 方法包括基于时间戳学生提交数据集的低分辨率和高分辨率实验，从语义、功能和风格维度评估代码输出。

Result: 结果显示微调显著提高了模型对学生学习轨迹的模拟能力，能更真实地捕捉错误模式、渐进改进和风格变化。

Conclusion: 结论表明，模拟真实学生代码需通过上下文感知生成、时间建模和多维评估来捕捉学习动态。
...
### [153] [FOUNDER: Grounding Foundation Models in World Models for Open-Ended Embodied Decision Making](https://arxiv.org/abs/2507.12496)
...
Conclusion: FOUNDER通过结合FMs和WMs，显著提升了任务泛化能力，并验证了奖励函数的一致性。
...
### [154] [VLMgineer: Vision Language Models as Robotic Toolsmiths](https://arxiv.org/abs/2507.12644)
*George Jiayuan Gao,Tianyu Li,Junyao Shi,Yihan Li,Zizhe Zhang,Nadia Figueroa,Dinesh Jayaraman*

Main category: cs.RO

TL;DR: VLMgineer利用视觉语言模型和进化搜索共同设计工具和操作策略，解决日常操纵任务，表现优于人工设计和现有工具。


<details>
  <summary>Details</summary>
Motivation: 探索基础模型是否能通过工具设计和使用的智能提升物理问题解决能力。

Method: 结合视觉语言模型的代码生成能力和进化搜索，迭代设计工具和操作策略。

Result: VLMgineer在多样化任务中表现优异，创新性解决挑战性问题。

Conclusion: VLMgineer展示了自动化工具设计的潜力，并计划公开基准和代码以推动研究。
...
### [155] [Rethinking the Embodied Gap in Vision-and-Language Navigation: A Holistic Study of Physical and Visual Disparities](https://arxiv.org/abs/2507.13019)
...
Method: 引入VLN-PE平台，评估多种技术方案，包括分类模型、扩散模型和基于LLM的无训练路径规划。

Result: 发现性能下降问题，主要由于机器人观测空间有限、环境光照变化和物理碰撞等挑战。
...
TL;DR: 论文提出了一种长期主动嵌入式问答（LA-EQA）任务，并设计了一种基于场景图的结构化记忆系统，结合探索与记忆检索，显著提升了机器人在复杂环境中的问答性能。


<details>
  <summary>Details</summary>
Motivation: 随着机器人操作时间的延长，如何利用累积的环境知识辅助人类成为关键问题。传统方法在长期记忆和主动探索方面存在局限，因此需要新的解决方案。

Method: 提出了一种结构化记忆系统，将经验编码为场景图，结合推理和规划算法，并引入基于信息价值的停止准则来平衡探索与记忆检索。

Result: 在真实世界实验和新基准测试中，该方法显著优于现有基线，提高了答案准确性和探索效率。

Conclusion: 该研究为长期环境中的机器人问答任务提供了有效解决方案，展示了结构化记忆和主动探索结合的优势。
...
### [157] [GraspGen: A Diffusion-based Framework for 6-DOF Grasping with On-Generator Training](https://arxiv.org/abs/2507.13097)
...
TL;DR: GraspGen 是一种基于 DiffusionTransformer 的抓取生成框架，通过迭代扩散过程提升抓取性能，并在模拟和真实机器人中表现优异。


<details>
  <summary>Details</summary>
Motivation: 尽管抓取是机器人的基本技能，但现有的基于学习的 6-DOF 抓取方法仍难以泛化到不同机械臂和复杂场景。

Method: 提出 GraspGen 框架，结合 DiffusionTransformer 架构和高效判别器，并引入新的训练方法。发布包含 5300 万次抓取的模拟数据集。

Result: 在模拟和真实机器人中表现优异，优于现有方法，并在 FetchBench 基准测试中达到最优。

Conclusion: GraspGen 通过迭代扩散和高效判别器显著提升了抓取性能，具有广泛的适用性。
...
### [158] [Physically Based Neural LiDAR Resimulation](https://arxiv.org/abs/2507.12489)
...
### [159] [Aligning Humans and Robots via Reinforcement Learning from Implicit Human Feedback](https://arxiv.org/abs/2507.13171)
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在稀疏奖励条件下效果不佳，且依赖复杂的手工设计奖励函数。现有RLHF方法需要显式反馈，增加了用户认知负担。

Method: 采用预训练解码器将EEG信号转化为概率奖励，结合稀疏外部奖励进行策略学习。在MuJoCo环境中用机械臂进行实验。

Result: 实验表明，基于EEG反馈训练的智能体性能接近手工设计密集奖励的效果。
...
### [160] [Evaluating Reinforcement Learning Algorithms for Navigation in Simulated Robotic Quadrupeds: A Comparative Study Inspired by Guide Dog Behaviour](https://arxiv.org/abs/2507.13277)
*Emma M. A. Harrison*

Main category: cs.RO

TL;DR: 研究比较了三种强化学习算法在四足机器人导航和避障中的表现，发现PPO算法表现最佳，为医疗辅助机器人提供了新思路。


<details>
  <summary>Details</summary>
Motivation: 探索四足机器人在医疗领域的潜力，特别是作为导盲犬的替代方案，填补相关研究空白。

Method: 通过对比PPO、DQN和Q-learning三种算法，在定制环境中评估其导航和避障能力。

Result: PPO算法在所有指标上表现最优，尤其在平均和每集目标步数上显著优于其他算法。

Conclusion: PPO算法在四足机器人导航中具有优势，为AI驱动的辅助机器人发展提供了重要参考。
...
### [161] [Latent Policy Steering with Embodiment-Agnostic Pretrained World Models](https://arxiv.org/abs/2507.13340)
*Yiqi Wang,Mrinal Verghese,Jeff Schneider*

Main category: cs.RO

TL;DR: 通过模仿学习视觉运动策略效果显著，但依赖大量训练数据。本文提出利用多体现数据（如公开机器人数据集或人类玩耍数据）减少数据需求，结合光流和世界模型优化策略。


<details>
  <summary>Details</summary>
Motivation: 减少真实世界中昂贵的数据收集需求，利用现有或低成本的多体现数据提升策略性能。

Method: 使用光流作为体现无关的动作表示训练世界模型，并在少量目标体现数据上微调；开发潜在策略导向（LPS）方法优化行为克隆策略。

Result: 在真实实验中，结合预训练世界模型，少量数据（30或50次演示）下策略性能显著提升（相对改进50%和20%）。

Conclusion: 通过多体现数据和世界模型预训练，显著减少了数据需求并提升了策略性能。
...
### [162] [Towards Formal Verification of LLM-Generated Code from Natural Language Prompts](https://arxiv.org/abs/2507.13290)
...
Method: 提出结合形式化查询语言表示用户意图，并通过符号解释器验证代码是否符合意图。
...
### [163] [Perfect diffusion is $\mathsf{TC}^0$ -- Bad diffusion is Turing-complete](https://arxiv.org/abs/2507.12469)
*Yuxi Liu*

Main category: cs.CC

TL;DR: 该论文研究了基于扩散的语言建模的计算复杂性，证明了基于评分网络质量的二分法：精确匹配评分函数的网络只能处理$\mathsf{TC}^0$复杂度任务，而无此限制的扩散模型可以模拟图灵机。


<details>
  <summary>Details</summary>
Motivation: 探讨扩散模型在语言建模中的计算能力与限制，为其理论能力提供框架。

Method: 通过分析评分网络的精确性与扩散模型的计算能力，提出二分法理论。

Result: 证明扩散模型在精确评分下受限于$\mathsf{TC}^0$复杂度，而无限制时可模拟图灵机。

Conclusion: 扩散模型的能力与评分网络质量相关，提出未来可能结合序列与并行模式的架构优于现有模型。
...
### [164] [A Semi-Supervised Learning Method for the Identification of Bad Exposures in Large Imaging Surveys](https://arxiv.org/abs/2507.12784)
...
TL;DR: 论文提出了一种基于机器学习的半监督流程，用于天文图像质量检测，结合了视觉变换器和k近邻分类器，成功应用于DECaLS数据。


<details>
  <summary>Details</summary>
Motivation: 传统的人工视觉检测方法在大规模天文图像调查中效率低下，需要更高效的自动化解决方案。

Method: 使用自监督学习的视觉变换器（ViT）结合k近邻分类器（kNN），构建半监督流程。

Result: 在DECaLS数据中识别出780个问题曝光，验证了方法的准确性和高效性。

Conclusion: 该方法为大规模天文图像调查提供了一种可扩展的质量控制解决方案。
...
### [165] [Quantum Transfer Learning to Boost Dementia Detection](https://arxiv.org/abs/2507.12485)
...
<details>
  <summary>Details</summary>
Motivation: 痴呆症的早期准确检测对干预和患者预后至关重要，但传统机器学习方法在高维生物医学数据和大规模数据集上存在计算和性能限制。

Method: 采用量子迁移学习（QTL）增强经典深度学习模型，应用于OASIS 2数据集的二元分类任务，并分析噪声对QTL的影响。

Result: 研究表明，量子技术能将次优的经典模型转化为更有效的生物医学图像分类解决方案。

Conclusion: 量子迁移学习在医疗技术中具有潜在应用价值，尤其在处理复杂数据时表现出优势。
...
### [166] [Sporadic Federated Learning Approach in Quantum Environment to Tackle Quantum Noise](https://arxiv.org/abs/2507.12492)
*Ratun Rahman,Atit Pokharel,Dinh C. Nguyen*

Main category: quant-ph

TL;DR: SpoQFL是一种新型量子联邦学习框架，通过动态调整训练策略以应对量子噪声异质性，显著提升模型性能和收敛稳定性。


<details>
  <summary>Details</summary>
Motivation: 量子噪声是现代量子设备中的主要问题，导致量子联邦学习（QFL）的训练性能不足。

Method: 提出SpoQFL框架，利用间歇性学习动态调整训练策略，以应对噪声波动。

Result: 实验表明，SpoQFL在真实数据集上优于传统QFL方法，性能显著提升且收敛更稳定。
...
### [167] [InSight: AI Mobile Screening Tool for Multiple Eye Disease Detection using Multimodal Fusion](https://arxiv.org/abs/2507.12669)
*Ananya Raghu,Anisha Raghu,Alice S. Tang,Yannis M. Paulus,Tyson N. Kim,Tomiko T. Oskotsky*

Main category: eess.IV

TL;DR: InSight是一款基于AI的应用，结合患者元数据和眼底图像，用于准确诊断五种常见眼病，提升筛查可及性。
...
Result: 图像质量检查器准确率近100%，多模态疾病诊断模型性能优于仅使用图像的模型（BRSET提升6%，mBRSET提升4%）。
...
### [168] [Improving Diagnostic Accuracy of Pigmented Skin Lesions With CNNs: an Application on the DermaMNIST Dataset](https://arxiv.org/abs/2507.12961)
...
<details>
  <summary>Details</summary>
Motivation: 色素性皮肤病变可能预示严重疾病如黑色素瘤，需高效分类方法以提升诊断准确性。

Method: 使用ResNet-50和EfficientNetV2L模型，结合迁移学习和不同层配置，对DermaMNIST数据集进行多分类。

Result: 部分配置结果优于现有方法，验证了CNN在生物医学图像分析中的潜力。

Conclusion: CNN可显著提升生物医学图像分析的诊断准确性，推动该领域进展。
...
### [169] [Pathology-Guided Virtual Staining Metric for Evaluation and Training](https://arxiv.org/abs/2507.12624)
*Qiankai Wang,James E. D. Tweel,Parsin Haji Reza,Anita Layton*

Main category: eess.IV

TL;DR: 论文提出了一种针对虚拟染色技术的新型评估指标PaPIS，通过结合深度学习和病理学特征，提高了评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法（如FR-IQA）和专家评审在虚拟染色技术中存在不足，无法准确捕捉病理学相关特征。

Method: 开发了PaPIS指标，利用深度学习特征和Retinex分解技术，专注于病理学感知的图像质量评估。

Result: PaPIS在实验中表现优于传统方法，能更准确地反映病理学特征，并提升虚拟染色模型的效果。

Conclusion: 病理学感知的评估框架对虚拟染色技术的发展至关重要。
...
### [170] [TRIQA: Image Quality Assessment by Contrastive Pretraining on Ordered Distortion Triplets](https://arxiv.org/abs/2507.12687)
*Rajesh Sureddi,Saman Zadtootaghaj,Nabajeet Barman,Alan C. Bovik*

Main category: eess.IV

TL;DR: 提出一种新的无参考图像质量评估方法，通过对比三元组学习构建自定义数据集，结合内容和质量特征进行预测。
...
Result: 模型在少量样本下表现优异，并在公开数据集上展现出强泛化性能。

Conclusion: 该方法通过高效训练和强泛化性能，推动了NR-IQA领域的发展。
...
### [171] [Pixel Perfect MegaMed: A Megapixel-Scale Vision-Language Foundation Model for Generating High Resolution Medical Images](https://arxiv.org/abs/2507.12698)
*Zahra TehraniNasab,Amar Kumar,Tal Arbel*

Main category: eess.IV

TL;DR: 论文提出了一种名为Pixel Perfect MegaMed的视觉语言基础模型，用于生成1024x1024分辨率的高分辨率医学图像，解决了传统生成模型在保留细粒度细节上的不足。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型（如GANs或VAEs）在高分辨率医学图像生成中难以保留对准确诊断至关重要的细粒度细节。

Method: 采用多尺度Transformer架构，结合针对医学术语和成像模态的视觉语言对齐技术，生成超高分辨率医学图像。

Result: 在CheXpert数据集上验证了模型能够从文本提示生成临床可信的胸部X光片，并在数据增强任务中表现出性能提升。

Conclusion: Pixel Perfect MegaMed在医学图像合成中实现了前所未有的分辨率和细节保留能力，为下游任务提供了实用价值。
...
### [172] [Unleashing Vision Foundation Models for Coronary Artery Segmentation: Parallel ViT-CNN Encoding and Variational Fusion](https://arxiv.org/abs/2507.12938)
*Caixia Dong,Duwei Dai,Xinyi Han,Fan Liu,Xu Yang,Zongfang Li,Songhua Xu*

Main category: eess.IV

TL;DR: 提出了一种基于视觉基础模型（VFMs）的并行编码架构，结合ViT和CNN编码器，通过交叉分支变分融合（CVF）模块和证据学习不确定性细化（EUR）模块，显著提升了冠状动脉分割的准确性。
...
Method: 采用ViT编码器捕获全局特征，CNN编码器提取局部细节，通过CVF模块融合特征，并使用EUR模块量化不确定性并优化分割结果。

Result: 在多个数据集上显著优于现有方法，展示了优异的性能和泛化能力。

Conclusion: 提出的框架在冠状动脉分割任务中表现卓越，代码已开源。
...
### [173] [From Variability To Accuracy: Conditional Bernoulli Diffusion Models with Consensus-Driven Correction for Thin Structure Segmentation](https://arxiv.org/abs/2507.12985)
...
<details>
  <summary>Details</summary>
Motivation: 眼眶骨CT图像分割对定制植入物至关重要，但现有方法在模糊边界和薄结构区域表现不佳，常出现不连续或欠分割结果。

Method: 采用条件伯努利扩散模型生成多个可能的分割结果，并通过共识驱动修正结合位置邻近性、共识水平和梯度方向相似性来优化模糊区域。

Result: 实验表明，该方法优于现有方法，显著提高了模糊区域的召回率，同时保持了薄结构的连续性。

Conclusion: 该方法自动化了分割结果修正过程，适用于图像引导的手术规划和手术。
...
### [174] [fastWDM3D: Fast and Accurate 3D Healthy Tissue Inpainting](https://arxiv.org/abs/2507.13146)
*Alicia Durrer,Florentin Bieder,Paul Friedrich,Bjoern Menze,Philippe C. Cattin,Florian Kofler*

Main category: eess.IV

TL;DR: 论文提出了一种结合DDPMs和GANs的2D图像生成方法，用于3D修复任务，并通过改进的噪声计划和重构损失实现了高质量快速修复。


<details>
  <summary>Details</summary>
Motivation: 解决现有DDPMs在健康组织修复中采样速度慢的问题，提高修复效率和质量。

Method: 采用方差保持噪声计划和特定重构损失，结合3D小波扩散模型（WDM3D），无需对抗训练。
...
### [175] [SpectraLift: Physics-Guided Spectral-Inversion Network for Self-Supervised Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2507.13339)
*Ritik Shah,Marco F. Duarte*

Main category: eess.IV

TL;DR: SpectraLift是一种自监督框架，通过融合低分辨率高光谱图像（LR-HSI）和高分辨率多光谱图像（HR-MSI），无需点扩散函数校准或地面真实数据，即可恢复高分辨率高光谱图像（HR-HSI）。
...
Method: 使用多光谱图像的谱响应函数（SRF）训练轻量级多层感知机（MLP），通过合成低分辨率多光谱图像（LR-MSI）和LR-HSI作为输入输出，优化光谱重建损失。
...
### [176] [An ultra-low-power CGRA for accelerating Transformers at the edge](https://arxiv.org/abs/2507.12904)
*Rohit Prasad*
...
Method: 采用4x4 PE阵列和4x2 MOB块，结合无交换的环形互连网络，优化并行计算和数据流。

Result: 该架构降低了内存带宽需求，提升了数据重用效率，适合边缘设备的资源限制。
...
### [177] [Tensor-Tensor Products, Group Representations, and Semidefinite Programming](https://arxiv.org/abs/2507.12729)
...
### [178] [Generative Multi-Target Cross-Domain Recommendation](https://arxiv.org/abs/2507.12871)
...
Method: GMC使用项目标记器生成领域共享语义标识符，将推荐任务建模为下一标记生成任务，并引入领域感知对比损失和领域特定微调。

Result: 在五个公共数据集上的实验表明，GMC优于多种基线方法。

Conclusion: GMC为MTCDR任务提供了一种有效的生成范式解决方案。
...
### [179] [UniSLU: Unified Spoken Language Understanding from Heterogeneous Cross-Task Datasets](https://arxiv.org/abs/2507.12951)
*Zhichao Sheng,Shilin Zhou,Chen Gong,Zhenghua Li*

Main category: eess.AS

TL;DR: UniSLU是一个统一框架，通过单一架构联合建模多个SLU任务，利用异构数据集并提升任务交互，实验证明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有SLU方法通常为每个任务设计独立模型，增加了复杂性且未能充分利用跨任务数据，限制了任务间的交互。

Method: 提出统一表示方法和生成式框架，联合建模ASR、NER和SA任务，结合大语言模型的生成能力。

Result: 在公开数据集上表现优于基准方法，适用于实际语音多媒体场景。

Conclusion: UniSLU框架有效解决了现有方法的局限性，提升了SLU任务的性能和实用性。
...
### [180] [LLM-Powered Quantum Code Transpilation](https://arxiv.org/abs/2507.12480)
*Nazanin Siavash,Armin Moin*

Main category: cs.SE

TL;DR: 论文探讨了利用大型语言模型（LLMs）作为量子软件开发工具包（QSDKs）之间的自动转译器，以解决跨平台开发的互操作性问题。


<details>
  <summary>Details</summary>
Motivation: 量子计算平台多样化的QSDKs（如Qiskit、Cirq等）导致互操作性和跨平台开发困难，传统基于规则的转译器设计维护成本高。

Method: 利用LLMs的预训练知识和上下文推理能力，将其作为编程语言无关的转译器，实现量子程序在不同QSDKs间的自动转换。

Result: 该方法无需手动定义转换规则，提供了可扩展的量子软件移植解决方案。

Conclusion: 研究为量子计算生态系统中智能、通用的转译迈出了重要一步。
...
### [181] [Kodezi Chronos: A Debugging-First Language Model for Repository-Scale, Memory-Driven Code Understanding](https://arxiv.org/abs/2507.12482)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型（LLMs）在代码生成和软件自动化中受限于有限的推理上下文和缺乏显式代码结构推理能力。

Method: Kodezi Chronos采用多级嵌入内存引擎，结合向量和图索引，实现高效代码检索和推理，支持仓库级理解和实时自我修复。

Result: 在新型Multi Random Retrieval基准测试中，Chronos表现优于现有模型，实际错误检测提升23%，调试周期减少40%。
...
### [182] [A Survey of AIOps in the Era of Large Language Models](https://arxiv.org/abs/2507.12472)
...
Method: 通过分析2020年至2024年的183篇研究论文，回答了四个关键研究问题（RQs），涵盖数据来源、任务演变、LLM方法和评估方法。

Result: 总结了LLM在AIOps中的最新进展、趋势和现有研究的不足。

Conclusion: 论文提出了未来研究的方向，以填补现有空白并推动LLM在AIOps中的进一步发展。
...
### [183] [QSpark: Towards Reliable Qiskit Code Generation](https://arxiv.org/abs/2507.12642)
*Kiana Kheiri,Aamna Aamir,Andriy Miranskyy,Chen Ding*

Main category: cs.SE

TL;DR: 论文研究了通过强化学习方法（GRPO和ORPO）优化量子电路代码生成，在Qiskit HumanEval基准测试中表现优于通用基线模型，但在高级任务上仍有改进空间。
...
Result: 在Qiskit HumanEval基准测试中，ORPO达到56.29% Pass@1，GRPO为49%，均优于通用基线；在原始HumanEval中分别为65.90%和63.00%。GRPO在基础任务表现更好，ORPO在中等任务更优，但均未解决高级任务。

Conclusion: 强化学习方法显著提升了量子编程辅助能力，但在高级任务上仍需进一步研究。
...
### [184] [A Fuzzy Approach to Project Success: Measuring What Matters](https://arxiv.org/abs/2507.12653)
*João Granja-Correia,Remedios Hernández-Linares,Luca Ferranti,Arménio Rego*

Main category: cs.SE

TL;DR: 提出了一种基于模糊逻辑的项目成功评估新方法，替代传统的Likert量表，更关注用户持续积极影响。
...
Method: 采用分层Type-1 Mamdani模糊系统，优先考虑用户持续积极影响，减少次要结果的权重。

Result: 动态方法可能更准确地衡量项目成功，并适用于复杂评估。
...
### [185] [Single Conversation Methodology: A Human-Centered Protocol for AI-Assisted Software Development](https://arxiv.org/abs/2507.12665)
*Salvador D. Escobedo*
...
<details>
  <summary>Details</summary>
Motivation: 当前对LLMs的依赖过于被动，SCM旨在重新确立开发者作为架构者和监督者的角色。

Method: SCM基于认知清晰性、可追溯性、模块化和文档化原则，定义开发阶段和最佳实践。

Result: SCM提供了一种结构化开发对话的方法，纠正了当前对LLMs的被动依赖。

Conclusion: SCM是一种必要的改进，强调开发者在LLMs使用中的主动性和结构化对话的重要性。
...
### [186] [Dual LiDAR-Based Traffic Movement Count Estimation at a Signalized Intersection: Deployment, Data Collection, and Preliminary Analysis](https://arxiv.org/abs/2507.13073)
*Saswat Priyadarshi Nayak,Guoyuan Wu,Kanok Boriboonsomsin,Matthew Barth*

Main category: eess.SY

TL;DR: 论文提出了一种基于双LiDAR系统的交通流量统计方法，用于优化信号灯控制和减少交通拥堵。


<details>
  <summary>Details</summary>
Motivation: 传统交通流量统计方法（如摄像头）在恶劣天气和夜间效果不佳，而LiDAR技术因其成本降低和广泛应用成为更优选择。
...
### [187] [Transforming Football Data into Object-centric Event Logs with Spatial Context Information](https://arxiv.org/abs/2507.12504)
...
Result: 基于真实足球数据生成了对象中心事件日志，并分析了不同过程表示的结果。
...
### [188] [Predictability-Aware Motion Prediction for Edge XR via High-Order Error-State Kalman Filtering](https://arxiv.org/abs/2507.13179)
...
<details>
  <summary>Details</summary>
Motivation: 探索6G网络如何通过低延迟和边缘基础设施支持XR应用的卸载，以优化用户体验和设备设计。

Method: 利用6G的低延迟和边缘处理能力，将计算密集型任务（如渲染）从用户设备迁移到网络。
...
### [189] [Bidirectional Age of Incorrect Information: A Performance Metric for Status Updates in Virtual Dynamic Environments](https://arxiv.org/abs/2507.13312)
*Chiara Schiavo,Manuele Favero,Alessandro Buratto,Leonardo Badia*

Main category: cs.NI

TL;DR: 本文提出双向错误信息年龄（BAoII）用于虚拟动态环境（VDEs）中实体交互的准确性评估，通过马尔可夫链模型优化更新策略。


<details>
  <summary>Details</summary>
Motivation: 虚拟动态环境（如元宇宙和数字孪生）需要准确表示交互实体，确保系统可靠性和无缝交互。

Method: 提出BAoII概念，扩展单向错误信息年龄，通过连续时间马尔可夫链模型推导长期BAoII的闭式解，并确定最优更新策略的传输成本阈值。

Result: 数值模拟验证了BAoII对系统性能评估的影响，揭示了通信成本与信息新鲜度之间的权衡。
...
### [190] [Wavelet-GS: 3D Gaussian Splatting with Wavelet Decomposition](https://arxiv.org/abs/2507.12498)
*Beizhen Zhao,Yifan Zhou,Sicheng Yu,Zijian Wang,Hao Wang*

Main category: cs.GR

TL;DR: 提出了一种将小波分解集成到3D高斯散射中的解耦优化框架，通过分别处理高频和低频分量来改善复杂场景重建中的结构完整性和光照效果问题


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯散射方法在复杂场景重建中面临挑战，表现为整体结构轮廓不完整和局部光照效果不清晰的问题

Method: 提出解耦优化框架，将小波分解集成到3D高斯散射和2D采样中。通过3D小波分解将点云分为高频和低频分量：低频分量捕获全局结构轮廓并通过体素化管理高斯分布；高频分量恢复精细几何和纹理细节，并结合重光照模块减轻光照伪影。同时对训练图像应用2D小波分解模拟辐射变化

Result: 在挑战性数据集上的大量实验表明，该方法在各种指标上达到了最先进的性能，超越了现有方法

Conclusion: 该方法成功解决了3D高斯散射在复杂场景重建中的关键问题，推进了3D场景重建领域的发展，实现了渲染质量、效率和速度的有效平衡
...
### [191] [HairFormer: Transformer-Based Dynamic Neural Hair Simulation](https://arxiv.org/abs/2507.12600)
*Joy Xiaoji Zhang,Jingsen Zhu,Hanyu Chen,Steve Marschner*

Main category: cs.GR

TL;DR: 提出了一种基于Transformer的两阶段神经网络方法，用于模拟任意发型、体型和动作下的头发动力学，实现了高保真和通用性。


<details>
  <summary>Details</summary>
Motivation: 模拟头发动力学在任意发型、体型和动作下的通用性是一个关键挑战，现有方法难以实现广泛的泛化能力。

Method: 采用两阶段神经网络：静态网络预测静态发型形状，动态网络通过跨注意力机制融合静态特征与运动输入，生成动态效果。支持实时推理和高效微调。

Result: 方法能够高保真地模拟各种发型的动态效果，解决穿透问题，并适用于未见过的复杂长发。

Conclusion: 该方法通过Transformer架构和物理约束损失，首次实现了广泛的头发动力学泛化能力。
...
### [192] [Implementation and Analysis of GPU Algorithms for Vecchia Approximation](https://arxiv.org/abs/2407.02740)
*Zachary James,Joseph Guinness*

Main category: stat.CO

TL;DR: 论文探讨了在GPU上实现Vecchia近似的方法，提出了一种新方法并优化了内存使用，最终在GpGpU R包中实现，性能优于现有方法。
...
Result: 新方法在运行时间和预测准确性上优于现有方法，特别是在处理大规模时空数据时。

Conclusion: GpGpU在GPU上高效实现了Vecchia近似，为大规模高斯过程建模提供了实用工具。
...
### [193] [Cross-Modal Watermarking for Authentic Audio Recovery and Tamper Localization in Synthesized Audiovisual Forgeries](https://arxiv.org/abs/2507.12723)
...
Method: 提出了一种跨模态水印框架，将真实音频嵌入到视觉内容中，以实现真实音频恢复和篡改定位。

Result: 实验表明，该方法在对抗多种伪造（如语音克隆和唇同步）时表现优异。

Conclusion: 该方法为视听伪造提供了有效的防御手段，能够恢复真实音频并定位篡改。
...
### [194] [Enkidu: Universal Frequential Perturbation for Real-Time Audio Privacy Protection against Voice Deepfakes](https://arxiv.org/abs/2507.12932)
...
Result: Enkidu在内存和运行效率上显著优于现有方法，并在实验中表现出高效防御能力。

Conclusion: Enkidu在防御语音深度伪造攻击方面具有高效性、通用性和实用性。
...
### [195] [Task-Specific Audio Coding for Machines: Machine-Learned Latent Features Are Codes for That Machine](https://arxiv.org/abs/2507.12701)
*Anastasia Kuznetsova,Inseon Jang,Wootaek Lim,Minje Kim*

Main category: cs.SD

TL;DR: 提出了一种高效的音频编码方法（ACoM），专注于机器任务而非人类感知，通过残差向量量化（RVQ）和任务特定损失实现超低比特率（<200 bps），同时保持下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统神经音频编解码器注重高保真重建，而音频编码为机器（ACoM）更关注高效压缩和下游任务性能，忽略感知细节。本文旨在填补这一空白。

Method: 采用任务特定损失和残差向量量化（RVQ）损失，压缩和量化已训练语音/音频下游模型的中间特征表示，支持灵活部署。

Result: 在自动语音识别和音频分类任务中验证，方法在超低比特率下保持性能，适用于多种任务和架构。

Conclusion: 该方法高效且灵活，适用于多种机器任务，展示了广泛的潜在应用前景。
...
### [196] [SHIELD: A Secure and Highly Enhanced Integrated Learning for Robust Deepfake Detection against Adversarial Attacks](https://arxiv.org/abs/2507.13170)
*Kutub Uddin,Awais Khan,Muhammad Umar Farooq,Khalid Malik*

Main category: cs.SD

TL;DR: 提出了一种名为SHIELD的协作学习方法，用于防御生成式反取证（AF）攻击，显著提高了检测准确性。


<details>
  <summary>Details</summary>
Motivation: 音频深度伪造（如deepfake）的传播带来严重风险，现有检测方法易受AF攻击。

Method: 结合防御生成模型（DF）和三重模型，通过协作学习捕获真实与AF攻击音频的关联。

Result: SHIELD在多种数据集上显著提升了检测准确性，平均准确率高达98%以上。

Conclusion: SHIELD能有效防御生成式AF攻击，具有鲁棒性和广泛适用性。
...
### [197] [Voxtral](https://arxiv.org/abs/2507.13264)
...
<details>
  <summary>Details</summary>
Motivation: 开发能够同时理解语音和文本的高性能模型，并在本地设备上高效运行。

Method: 训练多模态模型，支持32K上下文窗口，处理长达40分钟的音频和多轮对话。

Result: Voxtral Small在多个音频基准测试中表现优异，超越部分闭源模型。

Conclusion: Voxtral模型在多模态任务中表现卓越，并开源发布。
...
### [198] [Coarse Addition and the St. Petersburg Paradox: A Heuristic Perspective](https://arxiv.org/abs/2507.12475)
*Takashi Izumo*

Main category: econ.TH

TL;DR: 论文提出了一种基于粗粒度加法操作的替代方法，通过将数值分组为感知类别来解决圣彼得堡悖论，展示了在认知限制下如何处理无限期望值问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法通过引入辅助假设（如边际效用递减或时间折扣）解决圣彼得堡悖论，但这些方法可能不符合人类实际感知数值的方式。本文旨在探索一种更符合认知限制的替代方法。

Method: 提出了一种基于粗粒度分组的加法操作模型，将数值分组为感知类别，并用代表元素替代实际值进行加法运算，最终实现惯性稳定化。

Result: 研究表明，在适当的粗粒度分组下，圣彼得堡级数可以实现惯性稳定化，从而为有限认知能力的代理提供了一种处理无限期望值的可行方法。
...
### [199] [Scaling Up RL: Unlocking Diverse Reasoning in LLMs via Prolonged Training](https://arxiv.org/abs/2507.12507)
...
TL;DR: 通过强化学习优化小型语言模型在数学、编程和逻辑任务上的表现，提出关键训练技术并取得显著性能提升。
...
Method: 采用可验证奖励任务、改进GRPO算法，并引入KL正则化、剪裁比率和周期性参考策略重置等技术。

Result: 模型在数学、编程和逻辑任务上分别提升14.7%、13.9%和54.8%。

Conclusion: 提出的技术显著提升小型语言模型的性能，并公开模型以促进后续研究。
...
### [200] [Can Mental Imagery Improve the Thinking Capabilities of AI Systems?](https://arxiv.org/abs/2507.12555)
*Slimane Larabi*

Main category: cs.LG

TL;DR: 论文探讨了如何将心理意象整合到机器思维框架中，以增强自主推理能力，并提出了一种由认知思维单元和三个辅助单元组成的框架。
...
Method: 提出了一种机器思维框架，包含认知思维单元及三个辅助单元（输入数据单元、需求单元和心理意象单元），数据以自然语言或草图形式表示。

Result: 验证测试结果显示该框架在整合心理意象和机器思维方面具有潜力。
...
### [201] [Assay2Mol: large language model-based drug design using BioAssay context](https://arxiv.org/abs/2507.12574)
*Yifan Deng,Spencer S. Ericksen,Anthony Gitter*

Main category: cs.LG

TL;DR: Assay2Mol利用大型语言模型从生物化学筛选实验的非结构化文本中提取信息，用于早期药物发现，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 生物化学筛选实验的非结构化文本包含丰富信息，但未被充分利用。Assay2Mol旨在利用这些数据加速药物发现。

Method: Assay2Mol通过检索类似目标的实验记录，并利用上下文学习生成候选分子。

Result: Assay2Mol在生成候选配体分子方面优于现有机器学习方法，且生成的分子更易合成。

Conclusion: Assay2Mol为早期药物发现提供了一种高效的新方法，充分利用了现有实验数据。
...
### [202] [Learning What Matters: Probabilistic Task Selection via Mutual Information for Model Finetuning](https://arxiv.org/abs/2507.12612)
*Prateek Chanda,Saral Sureka,Parth Pratim Chatterjee,Krishnateja Killamsetty,Nikhil Shivakumar Nayak,Ganesh Ramakrishnan*

Main category: cs.LG

TL;DR: TASKPGM是一个用于优化大型语言模型（LLM）微调任务混合比例的框架，通过最小化马尔可夫随机场（MRF）上的能量函数，自动选择任务比例。
...
### [203] [BootSeer: Analyzing and Mitigating Initialization Bottlenecks in Large-Scale LLM Training](https://arxiv.org/abs/2507.12619)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM规模的扩大，启动开销（训练前的延迟）在工业级训练中变得日益重要，尤其是在频繁失败和多团队协作的场景下。

Method: 通过分析真实生产数据，识别启动开销的主要瓶颈，并设计Bootseer框架，采用热块记录预取、依赖快照和HDFS-FUSE分条技术。

Result: Bootseer在实际LLM训练中部署，减少了50%的启动开销。
...
### [204] [A Comprehensive Survey of Electronic Health Record Modeling: From Deep Learning Approaches to Large Language Models](https://arxiv.org/abs/2507.12774)
*Weijieying Ren,Jingxi Zhu,Zehao Liu,Tianxiang Zhao,Vasant Honavar*

Main category: cs.LG

TL;DR: 这篇论文综述了深度学习、大语言模型（LLMs）与电子健康记录（EHR）建模的最新进展，提出了一个统一的分类法，并讨论了开放挑战。
...
### [205] [Improving physics-informed neural network extrapolation via transfer learning and adaptive activation functions](https://arxiv.org/abs/2507.12659)
...
Method: 通过迁移学习在扩展训练域中使用少量选定的配置点，并提出自适应激活函数（线性组合标准激活函数）。

Result: 实验表明，该方法在外推域中平均减少40%的相对L2误差和50%的平均绝对误差，且计算成本未显著增加。

Conclusion: 该方法有效提升了PINNs的外推性能，同时保持了计算效率。
...
### [206] [PMKLC: Parallel Multi-Knowledge Learning-based Lossless Compression for Large-Scale Genomics Database](https://arxiv.org/abs/2507.12805)
*Hui Sun,Yanfeng Ding,Liping Yi,Huidong Ma,Gang Wang,Xiaoguang Liu,Cheng Zhong,Wentong Cai*

Main category: cs.LG

TL;DR: 提出了一种基于并行多知识学习的无损压缩器（PMKLC），解决了现有压缩器在压缩比、吞吐量和鲁棒性上的不足，并通过实验验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的无损压缩器在压缩比、吞吐量和鲁棒性方面表现不足，限制了其在基因组数据库备份、存储和传输中的广泛应用。

Method: 设计了自动多知识学习框架、GPU加速的(s,k)-mer编码器、数据块分区和逐步模型传递机制，并提供了单GPU和多GPU两种压缩模式。

Result: PMKLC在15个真实数据集上相比基线方法，压缩比提升最高达73.609%，吞吐量提升最高达10.710倍，同时具备最佳鲁棒性和内存效率。
...
### [207] [Probabilistic Soundness Guarantees in LLM Reasoning Chains](https://arxiv.org/abs/2507.12948)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM的错误检测方法未能有效处理早期错误对下游推理的负面影响，导致传播错误未被检测。

Method: 提出ARES框架，采用归纳方法为每个推理步骤生成细粒度评分，并提供统计保证。

Result: 在四个基准测试中表现最佳（Macro-F1 72.1%，提升8.2分），在长推理链中检测传播错误效果显著（F1 90.3%，提升27.6分）。

Conclusion: ARES通过避免错误传播，显著提升了LLM推理链的可靠性和鲁棒性。
...
### [208] [Data Transformation Strategies to Remove Heterogeneity](https://arxiv.org/abs/2507.12677)
...
<details>
  <summary>Details</summary>
Motivation: 数据异构性因多种冲突因素导致其利用复杂，现有方法多关注数据结构和模式冲突，忽视了数据转换的关键作用。随着AI广泛应用，对高效数据准备的需求增加，数据转换变得至关重要。

Method: 系统分类并分析了解决数据格式差异的策略，探讨了每种策略的挑战。

Result: 揭示了数据异构性的复杂性及其来源，提出了针对数据格式差异的解决方案。

Conclusion: 数据转换在AI应用中至关重要，需选择合适的转换技术以保留关键数据细节，当前缺乏对现代数据转换方法的全面综述。
...
### [209] [Teach Old SAEs New Domain Tricks with Boosting](https://arxiv.org/abs/2507.12990)
...
Method: 训练一个次级SAE，专门建模预训练SAE在领域特定文本上的重构误差，并在推理时结合两者的输出。

Result: 实验表明，该方法显著提升了跨熵和解释方差指标，同时保持了通用任务的性能。

Conclusion: 该方法能高效地将新领域知识融入现有SAE，为针对性机制解释提供了新可能。
...
### [210] [Inverse Reinforcement Learning Meets Large Language Model Post-Training: Basics, Advances, and Opportunities](https://arxiv.org/abs/2507.13158)
...
<details>
  <summary>Details</summary>
Motivation: 在大型语言模型（LLM）时代，对齐问题成为提升模型可靠性、可控性和能力的关键挑战。本文旨在通过逆向强化学习（IRL）探索LLM对齐的新方法。

Method: 文章首先介绍强化学习（RL）的基础概念，然后重点分析逆向强化学习（IRL）在LLM对齐中的应用，包括神经奖励模型的构建、数据集、评估指标等。

Result: 综述了当前研究的进展，指出了LLM对齐中的关键挑战和机遇，并提出了未来研究方向。

Conclusion: 通过综合多领域研究，本文为LLM对齐领域提供了结构化概述，并提出了未来改进RL和IRL技术的潜在方向。
...
### [211] [FLDmamba: Integrating Fourier and Laplace Transform Decomposition with Mamba for Enhanced Time Series Prediction](https://arxiv.org/abs/2507.12803)
*Qianru Zhang,Chenglei Yu,Haixin Wang,Yudong Yan,Yuansheng Cao,Siu-Ming Yiu,Tailin Wu,Hongzhi Yin*

Main category: cs.LG

TL;DR: FLDmamba框架结合傅里叶和拉普拉斯变换，解决了时间序列预测中多尺度周期性和瞬态动态的捕捉问题，并提升了模型对数据噪声的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 时间序列预测面临非平稳性、多尺度周期性和瞬态动态等挑战，现有Transformer和Mamba模型在效率和捕捉能力上存在不足。
...
### [212] [Supervised Fine Tuning on Curated Data is Reinforcement Learning (and can be improved)](https://arxiv.org/abs/2507.12856)
*Chongli Qin,Jost Tobias Springenberg*

Main category: cs.LG

TL;DR: 论文探讨了行为克隆（BC）在监督微调（SFT）中的重要性，并提出了一种改进方法iw-SFT，通过重要性加权优化RL目标。


<details>
  <summary>Details</summary>
Motivation: 研究SFT与强化学习（RL）之间的联系，探索如何通过改进SFT提升性能。
...
### [213] [MC$^2$A: Enabling Algorithm-Hardware Co-Design for Efficient Markov Chain Monte Carlo Acceleration](https://arxiv.org/abs/2507.12935)
...
Method: MC$^2$A通过扩展处理器性能模型分析工作负载多样性，提出参数化硬件加速器架构，并引入新型Gumbel采样器消除指数和归一化操作。

Result: 在端到端案例中，MC$^2$A相比CPU、GPU、TPU和现有MCMC加速器分别实现了307.6倍、1.4倍、2.0倍和84.2倍的加速。

Conclusion: MC$^2$A证明了通用硬件加速的可行性，为MCMC解决方案在多样化应用中的普及提供了支持。
...
### [214] [A Distributed Generative AI Approach for Heterogeneous Multi-Domain Environments under Data Sharing constraints](https://arxiv.org/abs/2507.12979)
...
Result: 在图像生成和分类指标上显著提升，生成分数提高1.1-2.2倍，分类指标平均提升10%（多域非IID设置下可达50%）。

Conclusion: 该方法在保护隐私的同时，有效利用分布式数据和低能力设备，显著提升了生成模型的性能。
...
### [215] [SMART: Relation-Aware Learning of Geometric Representations for Knowledge Graphs](https://arxiv.org/abs/2507.13001)
...
<details>
  <summary>Details</summary>
Motivation: 现有知识图谱嵌入模型未充分考虑关系特定的几何变换，限制了性能。

Method: 通过注意力机制为关系分配最佳几何变换，并在高低维空间中利用相关性优化嵌入。

Result: 在多个基准和真实金融知识图谱上表现优异，性能接近领先模型。

Conclusion: 提出的框架能有效提升知识图谱嵌入性能，尤其在关系特定变换的应用上表现突出。
...
### [216] [MUPAX: Multidimensional Problem Agnostic eXplainable AI](https://arxiv.org/abs/2507.13090)
...
<details>
  <summary>Details</summary>
Motivation: 开发一种鲁棒的XAI技术，要求其同时具备确定性、模型无关性和收敛性保证，以解决现有XAI方法在性能和解释性上的不足。

Method: MUPAX采用测度论框架，通过结构化扰动分析发现输入的内在模式并消除虚假关系，适用于任意维度和损失函数。

Result: 在音频分类、图像分类、医学图像分析和解剖标志检测等任务中，MUPAX表现出维度无关的有效性，并能提升模型准确性。

Conclusion: MUPAX是一种通用且高效的XAI技术，能够生成精确、一致且可理解的解释，为可信赖AI系统迈出重要一步。
...
### [217] [Multimodal-Guided Dynamic Dataset Pruning for Robust and Efficient Data-Centric Learning](https://arxiv.org/abs/2507.12750)
*Suorong Yang,Peijia Li,Yujie Liu,Zhiming Xu,Peng Ye,Wanli Ouyang,Furao Shen,Dongzhan Zhou*

Main category: cs.LG

TL;DR: 提出了一种动态数据集修剪框架，结合任务驱动难度和跨模态语义一致性，提升训练效率和模型性能。
...
Method: 利用预训练多模态基础模型监督，动态选择训练样本，过滤无信息样本。
...
### [218] [Merge Kernel for Bayesian Optimization on Permutation Space](https://arxiv.org/abs/2507.13263)
*Zikai Xie,Linjiang Chen*

Main category: cs.LG

TL;DR: 提出了一种基于排序算法的新型框架，用于生成排列空间上的核函数，其中Mallows核被视为冒泡排序的特例。进一步提出了复杂度更低的Merge Kernel，并通过实验验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 当前基于Mallows核的贝叶斯优化方法在排列空间中复杂度较高，需要一种更高效的核函数。

Method: 提出基于排序算法的框架，构造了复杂度为Θ(n log n)的Merge Kernel，并引入三种轻量级描述符提升鲁棒性。
...
### [219] [WaveletInception Networks for Drive-by Vibration-Based Infrastructure Health Monitoring](https://arxiv.org/abs/2507.12969)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在振动信号分析和基础设施健康监测中存在局限性，需要一种无需预处理且能高效提取多尺度特征的方法。
...
### [220] [DASViT: Differentiable Architecture Search for Vision Transformer](https://arxiv.org/abs/2507.13079)
...
Conclusion: DASViT为ViT架构搜索提供了一种高效且创新的方法，具有实际应用潜力。
